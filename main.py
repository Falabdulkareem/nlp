...
def answerq():
    ...
    # To do analysis for each Domain and output the result on Excel (Task3,4,5, and Experiment2)
    
    count = 0
    file = open('Experiment2/Apartment/S3P4.txt', 'r')
    workbook = xlsxwriter.Workbook('Experiment2/Apartment/S3P4Results.xlsx')
    ...
    for line in file:
        ...
        if ContainsRegex == '1':
            Preference, Goal, GoalForm, PrefForm = test_patterns(q, 4,
                                                    [   '(you|system|nurses|i|we) must (.*)',  
                                                        'must (complete|always) (.*)',
                                                        '(you may or may not|you may have|nurse[\w]? may|you may) (.*)',
                                                        '(don[\W]?t forget|don[\W]?t spend any[\s]?time) (.*)',
                                                        '(disregard|ignore entirely|skip|ignore) (.*)',
                                                        'not that [a-z\s]+ (to|you) (.*)',
                                                        'if there[\W]?( is|s) time left (.*)',
                                                        '[a-z\s\W]+ (achieve|achieving|complete|completing) (.*) or not',
                                                        '(every[\s]?time|urgently|immediately) (.*)',   
                                                        '(you[\W]?d|you would) better (.*)',
                                                        '(don[\W]?t|do not) bother (.*)',
                                                        'we (don[\W]?t|do not) care (.*)',
                                                    ])
            if Preference is None:
                Preference, Goal, GoalForm, PrefForm = test_patterns(q, 3,
                                                        [ 'upmost importance (.*)',
                                                          'you must try (.*)',
                                                         # '(you|system) must (.*)',  
                                                          '(you|participants|we|nurses?) (should definitely|should keep|should|need|can|would benefit) (.*)',
                                                          'it?[\W]?s most important (.*)',
                                                         # 'must (complete|always) (.*)',
                                                          'never ignore (.*)',
                                                          '(.*) must be [a-z\s]+',                                                  
                                                          ...
                                                          '(.*) should be received before most things',
                                                          'under no circumstance[\w]? should (.*)',
                                                          'under no circumstance[\w]? neglect (.*)',
                                                        ...
                                                        #  '[a-z\s\W]+ (achieve|achieving|complete|completing) (.*) or not',
                                                          '(.*) will (improve [a-z\s]+|make [a-z\s]+|be a bonus|not help [a-z\s]+|do no good)', 
                                                          '^if you can[\W]?t (.*)',
                                                          '(.*) (can|could|would|should) (be nice[a-z\s]*|be achieved[a-z\s]*|wait|be given[a-z\s]*|work|help[a-z\s]*|\
                                                          be finished[a-z\s]*|be completed[a-z\s]*|make us[a-z\s]*|be done[a-z\s]*|be great[a-z\s]*|(slight|slightly) be[a-z\s]*|be prioritized[a-z\s]*|\
                                                          be done as soon as possible|not be absolutely essential|be considered|be taken care of|be a risk|be a priority|be improved[a-z\s]*|be increased)',
                                                        #  '(don[\W]t forget|don[\W]t spend any[\s]?time) (.*)',
                                                          '(.*) to be completed[a-z\s]*',
                                                         # '(you may or may not|you may have) (.*)',                                                
                                                          '(.*) may or may not[a-z\s]*',
                                                          'if possible (.*)',
                                                          'if you get the chance (.*)',
                                                        #  'if there[\W]?( is|s) time left (.*)',
                                                          '(.*) doesn[\W]?t (have great significance|really matters?)',
                                                          '[a-z\s]* consider (.*)',
                                                        #  'not that [a-z\s]+ (to|you) (.*)',
                                                          '(.*) (is not that [a-z\s]+|are not[a-z\s]* important|not really required)',                                                 
            ...
            if Preference is not None:
                ...
                try:  
                    ...
                    cursor.execute("Select * from zero2 where Pref = %s", [Preference])
                    result = cursor.fetchone()
                    count = result[2]
                    count = count+1
                    print "this is the count"
                    print count
                    cursor.execute ("UPDATE zero2 SET Count=%s WHERE Pref=%s", (count,[Preference]))
                # if Preference is not in DB, INSERT it.
                except:
                    # Execute the SQL command
                    cursor.execute("INSERT INTO zero2(Pref, Count) VALUES (%s,%s)", (Preference,1))
                    # Commit your changes in the database
                    db.commit()
                # disconnect from server
                db.close()
                # write the Preference in File
                with open("Task3Pref/Fold2/0.txt", "a") as Pref_file:
        ...
        ### Return back after adding regex from Tasks 3,4,5 ###
        ...
        # Delete this after adding Regex from Tasks 3,4,5
        '''
        GoalsSimilarity = 0
        MatchingGoal = 0 
        SecondMatchingGoal = 0
        ThirdMatchingGoal = 0
        FourthMatchingGoal = 0 
        FifthMatchingGoal = 0
        PostP = 0
        MatchingGoalP = 0
        SecondMatchingGoalP = 0
        ThirdMatchingGoalP = 0
        FourthMatchingGoalP = 0
        FifthMatchingGoalP = 0
        FirstMatch = 0
        SecondMatch = 0 
        ThirdMatch = 0
        FourthMatch = 0
